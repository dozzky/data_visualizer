...
analysis_mode = st.radio(
    ...
    ["По путевым листам", "Построчно"],
    horizontal=True,
    index=0
)

if analysis_mode == "По путевым листам":
    df = df.groupby(["Ссылка", "Водитель", "Оборудование", "ДатаДокумента"], as_index=False).agg({
        "РасходТоплива": "sum",
        "ТехнологическиеОперацииКоличествоНоменклатурыВсего": "sum",
        "ТехнологическиеОперацииКоличествоОперацийВсего": "sum",
        "ТехнологическиеОперацииГрузооборот": "sum",
        "ПоказателиОборудованияПоУчасткамПродолжительность": "first"
    })
...
# === KPI по маршрутам (только построчный режим) ===
if analysis_mode == "Построчно":
    ...
    st.info("📌 Анализ маршрутов доступен только в режиме *Построчно*.")
...
if graph_type == "Scatter (точки)":
    ...
    if y_param in highlight_cols or x_param in highlight_cols:
        fig = px.scatter(
            ...
            x=x_param,
            y=y_param,
            color="ТехнологическиеОперацииВидРабот" if "ТехнологическиеОперацииВидРабот" in df.columns else None,
